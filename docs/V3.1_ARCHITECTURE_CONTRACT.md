# B5-1 Contract Change

> 本阶段只修改 `docs/V3.1_ARCHITECTURE_CONTRACT.md`，并追加 `PROJECT_V3.1_MASTER.md` 阶段状态。
> **未修改任何 `.py` 文件、前端、数据、Memory。**
> 由于无法直接写入仓库，以下为完整修改后文件内容，可直接替换。

---

## 1. 修改后的 `docs/V3.1_ARCHITECTURE_CONTRACT.md`

```markdown
# V3.1 Architecture Contract

> 版本：2026-09-26
> 状态：**FROZEN (with Contract Change CC-20260926-01)**
> 依据：PROJECT V3.1 MASTER + Phase A Global Inventory + B5-0 Context Contract Decision
...
| **AR-8** | **【CC-20260926-01 新增】** Runtime 的 CONTEXT 阶段输出类型冻结为 `AgentContext`（见 §5），不再返回 `ContextLayers` |
| **AR-9** | **【CC-20260926-01 新增】** Runtime 的 THINK 阶段输入类型冻结为 `AgentContext`，不再接收 `ContextLayers` |
| **AR-10** | **【CC-20260926-01 新增】** Runtime 不得同时调用旧 `ext_ai.build_ai_context` 与新 `ContextAssembler` |

### Context 阶段正式接口（CC-20260926-01 冻结）
...
### 数据来源冻结（CC-20260926-01）

| 参数 | 提供者 | 方式 |
|------|--------|------|
| `ai_name` | Runtime | `self.ai_name` |
| `owner` | Runtime | `get_state().owner` |
| `data` | Runtime | `self._data`（不透明引用） |
| `now_ts` | 调用方注入 | 缺省由 `ContextAssembler` 使用当前时间 |
| `agent_state_dict` | Runtime | `get_state().to_dict()` |
| `recall_query` | Runtime | 固定 `None`（本阶段） |
...
| **CA-6** | **【CC-20260926-01 新增】** `AgentContext` 是 V3.1 唯一正式 Context 数据结构，禁止新建平行 Context 结构 |
| **CA-7** | **【CC-20260926-01 新增】** `ContextLayers`（P0-3A）正式 **deprecated**，保留兼容 stub，不删除、不改名、不别名化；新 V3.1 链不再引用它 |
| **CA-8** | **【CC-20260926-01 新增】** 不允许长期存在"双 Context"：旧 `build_ai_context` 与新 `ContextAssembler` 不得在同一 AI 决策周期内同时触发 |
| **CA-9** | **【CC-20260926-01 新增】** `ContextAssembler` 只能被 `AgentRuntime.build_context()` 调用；其他调用方需 Contract 变更 |
| **CA-10** | **【CC-20260926-01 新增】** `decision_constraints` 当前保持 `None`，不得提前实现 Goal / Motivation / Commitment |

### ContextLayers 兼容状态（CC-20260926-01）

- `ContextLayers` 定义保留在 `agent/runtime.py`。
- 语义：**deprecated compatibility stub**。
- 不允许：
  - 删除；
  - 改名；
  - 别名化（如 `ContextLayers = AgentContext`）；
  - 新 V3.1 链引用。
...
| **ME-11** | **【CC-20260926-01 新增】** `LongTermProvider` 保持 stub，返回 `[]`；不因 Runtime 接入而启用旧 Memory Runtime |
...
- ❌ **【CC-20260926-01 新增】** 实现 Prompt Adapter
- ❌ **【CC-20260926-01 新增】** 实现 THINK 内部逻辑
...
- ❌ 修改 `ext_sms.py` 业务逻辑
- ❌ 修改 `ext_room.py` 业务逻辑
- ❌ **【CC-20260926-01 新增】** 删除 / 改名 / 别名化 `ContextLayers`
...
**本 Contract 变更（CC-20260926-01）允许修改**：

| 文件 | 内容 |
|------|------|
| `docs/V3.1_ARCHITECTURE_CONTRACT.md` | 本文件 |
| `PROJECT_V3.1_MASTER.md` | 追加阶段状态 |
...
- [x] B1 Context Data Contract 通过
- [x] B2-1 StableCoreProvider 通过
- [x] B2-2 DynamicWorldProvider 通过
- [x] B2-3 RecentProvider 通过
- [x] B2-4 LongTermProvider Stub 通过
- [x] B3 ContextAssembler 通过
- [x] B4 Runtime Connection Pre-Flight 通过
- [x] B5-0 Context Contract Decision 通过
- [ ] B5-1 Contract Change 通过（本阶段）
...
## 23. Contract Change Log

### CC-20260926-01 — B5-1 Context Contract Change

**日期**：2026-09-26
**阶段**：B5-1
**依据**：B5-0 Context Contract Decision（ACCEPTED）
**审核**：架构审核通过

**变更摘要**：

1. **V3.1 正式 Context 标准**：
   - `AgentContext`（`agent/context.py`）成为 V3.1 唯一正式 Context 数据结构。
   - 新增规则：CA-6。

2. **`ContextLayers` 命运**：
   - 正式 **deprecated**。
   - 保留兼容 stub，不删除、不改名、不别名化。
   - 新 V3.1 链不再引用。
   - 新增规则：CA-7。
...
4. **唯一正式数据链**：
   ```text
   Event → RECEIVE → PERCEIVE → WAKE_DECISION → CONTEXT
       → AgentRuntime.build_context()
       → ContextAssembler
       → AgentContext
       → THINK（仍 Stub）
...
6. **保持不变**：
   ...
   - 不实现 Goal / Motivation / Commitment / Activity / World Query / Scheduler / Brain / Prompt Adapter。
   - 不接 LLM / Decision / Action。
   - 新增规则：ME-11、CA-10。

**变更影响面**：
- 仅影响 V3.1 Context 契约语义。
- 不修改任何 `.py`。
- 不修改任何业务逻辑。
- Runtime 实现待 B5-2 阶段执行。

**实施顺序**：
1. B5-1（本阶段）：Contract Change。
2. B5-2：Runtime Connection。
3. B5-3：测试。
4. B5-4：PROJECT 更新。

---

**V3.1 Architecture Contract 已冻结（含 CC-20260926-01）。**

**下一步：等待架构审核通过后，进入 B5-2 Runtime Connection。**
```
